...
func MappingRoutes(app *fiber.App) {
	...
	api.Get("/mappings/by-odp-name/:name", middleware.JWTProtected(), controllers.GetRouterByODPName)

	// CRUD Mappings
	api.Get("/mappings", middleware.JWTProtected(), controllers.GetTopologyMappings)
	api.Get("/mappings/:id", middleware.JWTProtected(), controllers.GetTopologyMapping)
